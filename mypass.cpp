...
struct SwPrefetchPass : FunctionPass, InstVisitor<SwPrefetchPass>
{
    ...
    bool canIncludeFunctionCall(CallInst *call){
        ...
        //Pass *P = new TargetTransformInfoWrapperPass();
        //errs()<<P->getPassID()<<'\n';
        //DM->recordAvailableAnalysis(P);
        auto *TTIWP = getAnalysisIfAvailable<TargetTransformInfoWrapperPass>();
        TargetTransformInfo *TTI=TTIWP ? &TTIWP->getTTI(*fun) : nullptr;
        for (inst_iterator I = inst_begin(fun), E = inst_end(fun); I != E; ++I){
            Instruction* inst= &*I;
            if (dyn_cast<CallInst>(inst)!=nullptr||dyn_cast<LoadInst>(inst)!=nullptr||dyn_cast<StoreInst>(inst)!=nullptr||dyn_cast<BranchInst>(inst)!=nullptr){
            	return false;
            }
            cost+=TTI->getInstructionCost(inst, TargetTransformInfo::TargetCostKind::TCK_RecipThroughput);
            errs()<<cost<<'\n';
        }
        errs()<<cost<<'\n';
        return (cost<=12);
    ...
    bool depthFirstSearch (Instruction* I, LoopInfo &LI, Instruction* &Phi, SmallVector<Instruction*,8> &Instrs, SmallVector<Instruction*,4> &Loads, SmallVector<Instruction*,4> &Phis, std::vector<SmallVector<Instruction*,8>>& Insts) {
        ...
        for(Use* v = u; v<end; v++) {
            ...
            else if(CallInst *k=dyn_cast<CallInst>(v->get())) {
                // k->print(errs());
                // errs()<<'\n'<<p<<' '<<L<<' '<<Phi<<'\n';
                if(!((!p) || L != nullptr)) continue;

                if (!canIncludeFunctionCall(k)) continue;
            ...
            else if(dyn_cast<ReturnInst>(v->get())){}
            else if(dyn_cast<BranchInst>(v->get())){}
            else if(dyn_cast<SwitchInst>(v->get())){}
    ...
    bool runOnFunction(Function &F) override {
        ...
        for(uint64_t x=0; x<Loads.size(); x++) {
            ...
            SmallVector<CallInst*,8> callInsts;
            ...
            for(auto &z : newInsts) {
                ...
                if(z == Phis[x]) {
                    ...
                    if(weird) {
                        ...
                        Value* sub = Builder.CreateSub(cast,ConstantInt::get(n->getType(),1));
                    ...
                    // errs()<<F.getParent()->getContext();
                    // SmallVector<Intrinsic::IITDescriptor, 8> Table;
                    // getIntrinsicInfoTableEntries(Intrinsic::prefetch, Table);
                    // for (auto &h:Table){
                    //     errs()<<h.Kind<<'\n';
                    //     if (h.Kind==Intrinsic::IITDescriptor::Argument) errs()<<h.getArgumentNumber()<<'\n';
                    // }
                    // errs()<<Table.size()<<'\n';
                    Function *fun = Intrinsic::getDeclaration(F.getParent(), Intrinsic::prefetch, {Type::getInt32PtrTy(M->getContext())});
                    ...
                    Instruction* cast = dyn_cast<Instruction>(Builder.CreateBitCast (gep, Type::getInt32PtrTy(M->getContext())));
                    ...
                    Transforms.insert(std::pair<Instruction*,Instruction*>(z,n));
                    modified = true;
                    if (CallInst* callInst=dyn_cast<CallInst>(n)){
...
static RegisterPass<SwPrefetchPass> X("hello", "Hello World Pass");
