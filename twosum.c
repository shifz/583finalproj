...
int* twoSum(int* nums, int numsSize, int target, int* returnSize){
    ...
    for(i = 0;i < numsSize; i++){
        //hash_insert(nums[i]);
        int addr = hash(nums[i]);         /*呼叫雜湊函數得位置*/
    ...
    {
        //hash_srch
        idx=hash(target-nums[i]); //得到key的address
        while(table[idx] != (target-nums[i]) )   //當addr的值不等於key 往右移一格
        {
            idx=(idx+1)%m;  //取除m餘數確保可循環
            if(table[idx]== EMPTY || idx==hash(target-nums[i])){
                idx = -1;
                break;
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    int target = nums[701] + nums[999];
    int return_size = 2;

    for (int j=0;j<100000;j++){
    int* result = twoSum(nums, 1000, target, &return_size);
    free(result);
    }
    //printf("%d ", result[0]);
    //printf("%d", result[1]);
